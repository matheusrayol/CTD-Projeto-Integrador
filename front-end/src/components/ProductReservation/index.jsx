...
export default function ProductReservation() {
    ...
    const finalDate = (range) => {
        if (!range) {
            return '';
        }
        const finalStartDay = range[0].toLocaleString('pt-BR', { day: '2-digit' });
        const finalStartMonth = range[0].toLocaleString('pt-BR', { month: '2-digit' });
        const finalStartYear = range[0].toLocaleString('pt-BR', { year: 'numeric' });
        const finalEndDay = range[1].toLocaleString('pt-BR', { day: '2-digit' });
        const finalEndMonth = range[1].toLocaleString('pt-BR', { month: '2-digit' });
        const finalEndYear = range[1].toLocaleString('pt-BR', { year: 'numeric' });

        setFinalDateStart(`${finalStartYear}-${finalStartMonth}-${finalStartDay}`)
        setFinalDateEnd(`${finalEndYear}-${finalEndMonth}-${finalEndDay}`)
    }
                                    ...
                                    <div className="col-12 col-lg-7 px-4 py-4 rounded bg-white">
                                        ...
                                        <div className="d-flex mb-5">
                                            <select 
                                                ...
                                                {...register('pickupTime', {
                                                    required: "É necessário selecionar um horário de retirada."
                                                })}
                                                >
                                    ...
                                    <div className="col-12 col-lg-5 d-flex flex-column flex-grow-0 flex-shrink-1 align-self-start rounded shadow-lg px-4 py-4 bg-success">
                                        ...
                                        <section className="py-2">
                                            ...
                                            <input
                                                ...
                                                defaultValue={product.city.name}
                                                {...register('city', { required: "A cidade deve ser informada." })}
                                            />
                                            <input
                                                id="inicioReserva"
                                                type="hidden"
                                                ...
                                                {...register('inicioReserva', { required: "A data de início da reserva deve ser informada." })}
                                            />
                                            {register("inicioReserva", { value: finalDateStart })}
                                            <input
                                                id="fimReserva"
                                                type="hidden"
                                                name="fimReserva"
                                                value={finalDateEnd}
                                                {...register('fimReserva', { required: "A data de fim da reserva deve ser informada." })}
                                            />
                                            {register("fimReserva", { value: finalDateEnd })}
